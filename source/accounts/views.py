...
import csv
from .test_data_download import callableFunTest
from .train_data_download_many import callableFun
...
def GetTrainingData(request):
    ...
    if request.method == "POST":
        ...
        saved = True
        if saved:
            messages.success(request, _('Training Data Downloaded Successfully'))
        else:
            messages.success(request, _('Failed to download Training Data File'))

    return render(request, 'analyzer.html', locals())


from .run_classifier import getRecommendation
...
def getRecommendations(request):
    ...
    if request.method == "GET":
        ...
        try:
            ...
            print(e)
        print(output)
        # context = {'loaded_data': output}
        saved = True
        if saved:
            messages.success(request, _('Recommendation Successfull'))
        else:
            messages.success(request, _('Failed to download make Recommendations'))

        return render(request, 'show_recommendation.html', output)


from .test_data_download import getUsersPlaylist1
...
def GetPlayList(request):
    ...
    if request.method == "GET":
        output = getUsersPlaylist1()
        print(output)
        # context = {'loaded_data': output}

        return render(request, 'playlist.html', output)


from .test_data_download import getArtistAlbums


def getPlaylistsArtist(request):
    saved = False

    if request.method == "GET":
        artist_id = request.GET.get('artist_id', '')

        output = getArtistAlbums(artist_id)
        print(output)
        # context = {'loaded_data': output}

        return render(request, 'artist_albums.html', output)


from .test_data_download import getTracksFromPlaylist
...
def GetTrackList(request):
    if request.method == "POST":
        ...
        print(playlist)
        output = getTracksFromPlaylist(playlist)
        # output={}
        print(output)
        # context = {'loaded_data': output}
        return render(request, 'track_list.html', output)
...
def GetTestData(request):
    ...
    if request.method == "POST":
        ...
        saved = True
        if saved:
            messages.success(request, _('Test Data Downloaded Successfully'))
        else:
            messages.success(request, _('Failed to download Test Data File'))
...
def viewTrainingData1(request):
    data = pd.read_csv("source/content/media/test.csv")
...
def viewTrainingData(request):
    data = pd.read_csv("source/content/media/test.csv")
    # data_html = data.to_html()
    data_html = data.to_html()
    df = pd.DataFrame(data)

    context = {'loaded_data': df.to_html(table_id='hello', classes='table-striped table-bordered table-sm')}
    return render(request, "tables.html", context)


def GetTrainingData1(request):
    saved = False
